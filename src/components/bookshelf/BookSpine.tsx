...
export default function BookSpine({ book, index, onClick }: BookSpineProps) {
  ...
  useEffect(() => {
    if (!showPreview || !spineRef.current) {
      setPopupPos(null);
      return;
    }
    ...
    setPopupPos({
      ...
      y: below ? rect.bottom + 16 : rect.top - 16,
      below,
    });
  }, [showPreview]);
  ...
  return (
    <>
      ...
      {/* Portal popup — rendered on document.body so never clipped */}
          ...
          <AnimatePresence>
            {showPreview && popupPos && (
              <motion.div
                ...
                style={{
                  left: popupPos.x,
                  top: popupPos.below ? popupPos.y : undefined,
                  bottom: popupPos.below
                    ? undefined
                    : window.innerHeight - popupPos.y,
                  transform: "translateX(-50%)",
                }}
              >
                <div className="bg-warm-50 border border-warm-200 rounded-xl p-3 shadow-xl min-w-[180px]">
                  {book.coverUrl ? (
                    ...
                    >
                      <span
                        ...
                        style={{
                          color: lightenColor(book.spineColor, 140),
                        }}
                      >
                        {book.title}
                      </span>
